...
export const formatPostDateToString = (post?: Pick<WPPost, 'date'>, format = 'YYYY/MM/DD') => {
    if (!post || !post.date) return ''
    return dayjs(post.date).format(format)
}

export const getThePostFeatureImage = (post?:Pick<WPPost, '_embedded'>, size: keyof WPMediaDetailSizes = 'thumbnail'): WPMediaDetailSize & {
    alt_text?: string;
} | undefined => {
...
export const getThePostAuthor = (post?: Pick<WPPost, '_embedded'>, size: '24' | '48' | '96' = '48')  => {
